...
.replace <- function(dataset, var, rpl,
  ...
  if (!store || !is.character(dataset)) {
    ...
    if (store_dat == "") store_dat <- dataset
    paste0("## replace variable(s)\nr_data[[\"",store_dat,"\"]] <- mutate(r_data[[\"",dataset,"\"]], ", paste(var, rpl, sep = " = ", collapse = ", "), ") %>% dplyr::select(", paste0("-",rpl, collapse = ", "),")\n") %>%
      update_log(input$tr_log)
  }
}
...
.reorg_vars <- function(dataset,
  ...
  if (!store || !is.character(dataset)) {
    ...
    if (store_dat == "") store_dat <- dataset
    paste0("## reorder/remove variables\nr_data[[\"",store_dat,"\"]] <- dplyr::select(r_data[[\"",dataset,"\"]], ", paste0(vars, collapse = ", "),")\n") %>%
      update_log(input$tr_log)
  }
}
