...
.create <- function(dataset, cmd,
  ...
  if (!store || !is.character(dataset)) {
    if (gsub("\\s","",cmd) == "") return(dataset)
        ...
        cmd <- cmd %>% gsub("\"","\'",.) %>% gsub(" ","",.)
      vars <-
        base::strsplit(cmd, ";")[[1]] %>% base::strsplit(., "=") %>%
        sapply("[", 1) %>% gsub(" ","",.)
      dots <- base::strsplit(cmd, ";")[[1]] %>% gsub(" ","",.)
      ...
      if (is_empty(byvar)) {
        ...
        nvar <- try(group_by_(dataset, .dots = byvar) %>% dplyr::mutate_(.dots = setNames(dots, vars)), silent = TRUE)
      }
      if (is(nvar, 'try-error')) {
        ...
        dplyr::select_(nvar, .dots = vars) %>% ungroup
  ...
  .replace <- function(dataset, var, rpl,
    ...
    if (!store || !is.character(dataset)) {
      dplyr::select_(dataset, .dots = rpl) %>% set_colnames(var)
    } else {
      if (store_dat == "") store_dat <- dataset
  ...
  .remove_na <- function(dataset,
    ...
    if (!store || !is.character(dataset)) {
      if (all(vars == "") || length(unique(vars)) == ncol(dataset)) {
        ...
        ind <- dplyr::select_(dataset, .dots = vars) %>% complete.cases
        filter(dataset, ind)
      }
    } else {
      if (store_dat == "") store_dat <- dataset
  ...
  transform_main <- reactive({
    ...
    if (input$tr_change_type == "none")
      return(dplyr::select_(dat, .dots = input$tr_vars))
