...
#       dplyr::select(isNum) %>% mutate_each("as.numeric") %>%
#       summarise_each(pfun)
#
...
#         dplyr::select_(.dots = c(byvar, "variable", names(pfun)))
#     }
# #>>>>>>> upstream/master
#   }
#
...
#   if (sum(isPeriod) > 0) {
#     cat("Earliest time:\n")
#     dplyr::select(dat, which(isPeriod)) %>% summarise_each(funs(min)) %>% print
#     cat("\nFinal time:\n")
#     dplyr::select(dat, which(isPeriod)) %>% summarise_each(funs(max)) %>% print
#     cat("\n")
#   }
#   if (sum(isChar) > 0) {
# # <<<<<<< HEAD
...
#     dplyr::select(dat, which(isChar)) %>% distinct %>% lapply(unique) %>%
